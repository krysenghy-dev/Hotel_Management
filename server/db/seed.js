...
function seedRooms() {
  ...
  const rooms = [
    ...
    { num: 304, floor: 3, type: 'Suite', status: 'Available', price: 450 },
...
function seedGuests() {
  ...
  if (count > 0) return;
  const guests = [
    { name: 'Eleanor Shellstrop', code: 'G-1001', email: 'eleanor@example.com', phone: '+1 (555) 123-4567', room: 402, status: 'Checked In', visit: '2023-10-12' },
    { name: 'Chidi Anagonye', code: 'G-1002', email: 'chidi@example.com', phone: '+1 (555) 234-5678', room: 205, status: 'Checked In', visit: '2023-10-12' },
    ...
    { name: 'Janet Della-Denunzio', code: 'G-1006', email: 'janet@example.com', phone: '+1 (555) 678-9012', room: 208, status: 'Reserved', visit: '2023-01-15' },
  ...
  const tx = db.transaction((list) => list.forEach(g => {
    // room 208 doesn't exist in the seed room list (matches original demo data quirk) -> room_id NULL if not found
...
function seedBookings() {
  ...
  const bookings = [
    { name: 'Eleanor Shellstrop', code: 'BK-7829', room: 402, type: 'Suite', in: `${year}-10-12`, out: `${year}-10-15`, amount: 1250, status: 'Confirmed' },
    { name: 'Chidi Anagonye', code: 'BK-7830', room: 205, type: 'Single', in: `${year}-10-12`, out: `${year}-10-14`, amount: 450, status: 'Confirmed' },
    { name: 'Tahani Al-Jamil', code: 'BK-7831', room: 501, type: 'Deluxe', in: `${year}-10-13`, out: `${year}-10-20`, amount: 3800, status: 'Pending' },
    { name: 'Jason Mendoza', code: 'BK-7832', room: 112, type: 'Double', in: `${year}-10-14`, out: `${year}-10-16`, amount: 600, status: 'Cancelled' },
  ...
  const tx = db.transaction((list) => list.forEach(b => {
    insert.run(b.code, b.name, roomIdByNumber(b.room), b.type, b.in, b.out, b.amount, b.status);
  }));
  tx(bookings);
